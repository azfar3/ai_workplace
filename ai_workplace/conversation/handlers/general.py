...
class GeneralHandler:
    def can_handle(self, intent: str, state: str) -> bool:
        return intent in (
            "help", "hr_pin_help", "my_day", "supervisor_reporting", 
            "guest_number_changed", "contact_hr", "guest_contact", 
    ...
    def handle(self, conv: Any, intent: str, clean_text: str, context: Dict[str, Any], trace_id: str) -> Optional[OutboundMessage]:
        ...
        if intent == "help":
        ...
        elif intent in ("contact_hr", "guest_contact"):
        ...
        elif intent in ("concerns", "guest_concern", "former_concern"):
            ...
            from ai_workplace.services.dashboard import build_my_day_response
            update_conversation(conv, state=ConversationState.AWAITING_SELECTION, current_intent=intent, active_service=None)
            resp_text = build_my_day_response(context)
            outbound = wrap_with_menu_again(resp_text, context)
            action = "view_my_day"
            
        elif intent == "supervisor_reporting":
            from ai_workplace.services.dashboard import build_supervisor_reporting_response
            update_conversation(conv, state=ConversationState.AWAITING_SELECTION, current_intent=intent, active_service=None)
            resp_text = build_supervisor_reporting_response(context)
            outbound = wrap_with_menu_again(resp_text, context)
            action = "view_supervisor_reporting"
            
        elif intent == "guest_number_changed":
